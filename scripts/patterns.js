...
$(function() {
  ...
  $('.b-ui-library__item-link.expandable').on('click', function(){
    if( $(this).hasClass('open') === true ) {
      ...
      return !$(this).hasClass('b-ui-library__item-link--selected') && $(this).hasClass('b-ui-library__item-link--has-content');
    }
  });
  
  // Highlight Syntax-Coloring Configuration
  hljs.configure({
    tabReplace: '  '
  });
  
  // Highlight Syntax-Coloring Trigger
  ...
  $(contactFormInput).change(function() {
    ...
    if ( $(this).is( "input[type=email]" ) ) {
      $(this).filter(function(){
        ...
        var emailReg = /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/;
        if( !emailReg.test( email ) ) {
          $(this).siblings('.<img src="./image/placeholder.png" alt="a placeholder image" />').html('<span style="color:red">Email seems invalid</span');
          emailvalid = false;
        } else {
          $(this).siblings('.<img src="./image/placeholder.png" alt="a placeholder image" />').html('Email');
          emailvalid = true;
